...
describe("Add new observation", () => {
  ...
  it("Has a Notes textarea", () => {
    const { getByTestId, getByRole } = render(<AddNewObservation currentId="" />);
  ...
  it("Has an optional setup name", () => {
    ...
    expect(getByRole('textbox', { name: /setup name \(optional\)/i })).toBeTruthy();
  })

  it("Calls Add or Edit when submitted", () => {
    const { getByRole } = render(<AddNewObservation
                                  currentId=""
                                  addOrEdit={addOrEdit}
