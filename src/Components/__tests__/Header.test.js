...
describe("Header", () => {
    ...
    it("Shows email once logged in", () => {
        const { getByTestId } = renderComponent({user: {userId: 'Test User', userEmail: 'test@test.com'}});
        expect(getByTestId('email').textContent).toContain("test@test.com");
    })

    it("Shows a button to log out", () => {
        const { getByRole } = renderComponent({user: {userId: 'Test User', userEmail: 'test@test.com'}});
